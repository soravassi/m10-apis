...
title <- paste0('t=', move.no.spaces)
parameters <- paste(title, sep = '&')
...
Director <- function(movie) {
  ...
  answer <- paste(title, 'was directed by', director)
  return(answer)
}
...
movie.list <- c('Inception', 'Memento', '')
